...
export function App({configuration}: {configuration: InitializerConfig}): ReactElement {
    ...
    const [coreModules, setCoreModules] = useState<string[]>([]);
                ...
                <div className="configuration">
                    <section className="panel"><h2><span>01</span> Project</h2><label htmlFor="game-name">Game name</label>
                        ...
                        <div className="icon-editor"><img src={iconPreview} alt="Game icon preview"/><div><label htmlFor="game-icon">Game icon</label>
                            ...
                            <p className="help">Centered, cropped and saved as an opaque 1024×1024 PNG for Android and iOS.</p>
                    ...
                    <section className="panel"><h2><span>02</span> Platforms & runtime</h2><div className="platform-options">
                        ...
                        <label htmlFor="java-version">Java version</label><select id="java-version" value={javaVersion} onChange={event => setJavaVersion(Number(event.target.value))}>
                            {configuration.javaVersions.map((value: JavaVersionDefinition) => <option value={value.version} key={value.version}
                                disabled={!supportsJavaVersion(configuration, value.version, platforms)}>{value.name}{value.recommended ? ' — recommended' : ''}</option>)}</select>
                        <p className="help">Incompatible Java versions remain visible and disabled.</p>
                    ...
                    <section className="panel"><h2><span>03</span> Renderer</h2><label htmlFor="renderer" className="sr-only">Renderer</label>
                        <select id="renderer" value={renderer} onChange={event => setRenderer(event.target.value)}>
                            {configuration.renderers.map((value: RendererDefinition) => <option value={value.id} key={value.id}
                                disabled={!supportsRenderer(configuration, value.id, platforms)}>{value.name}{value.recommended ? ' — recommended' : ''}</option>)}</select>
                        ...
                        <p className="help">Incompatible renderer choices remain visible and disabled.</p></section>
                </div>
                <div className="module-column">
                    <section className="panel core-modules-panel"><div className="library-heading"><h2><span>04</span> Core modules</h2><span className="selection-count">{coreModules.length} selected</span></div>
                    ...
                    <section className="panel allocator-panel"><h2><span>05</span> Native memory allocator</h2><div className="allocator-list">
